...
const badgeClass = (status: string) => {
  if (status === 'emergency_flagged') return 'border-critical/25 bg-critical/10 text-critical';
  if (status === 'video_link_created') return 'border-primary/25 bg-primary/10 text-primary';
  if (status === 'started') return 'border-warning/25 bg-warning/10 text-warning';
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-5 text-sm text-muted-foreground">
        <p className="font-semibold text-foreground">Safety disclaimer</p>
        <p className="mt-1">Video consultation is provided by the participating healthcare provider. ChekaMeds does not diagnose, prescribe, or replace emergency care.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {requests.map((request) => {
            ...
            return (
              <article key={request.id} className={`rounded-2xl border bg-card p-5 shadow-sm ${isEmergency ? 'border-critical/40 ring-2 ring-critical/10' : 'border-border'}`}>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[requestStatus, consultationStatus].filter(Boolean).map((status) => (
                      <span key={status} className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${badgeClass(status)}`}>
                ...
                {isEmergency && (
                  <div className="mt-4 rounded-xl border border-critical/20 bg-critical/10 p-3 text-sm text-critical">
                  ...
                  <div className="rounded-2xl border border-border bg-muted/30 p-4">
                    ...
                    {!request.video_room_url ? (
                      <div className="space-y-3">
                        <p className="text-xs text-muted-foreground">Only facility/admin users can create video consultation links.</p>
                        <button
                          onClick={() => createVideoMutation.mutate(request.id)}
                          disabled={createVideoMutation.isPending}
                          className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground transition hover:bg-primary/90 disabled:opacity-60"
                        >
                      ...
                      <div className="space-y-3">
                        <a href={request.video_room_url} target="_blank" rel="noreferrer" className="block break-all rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs font-medium text-primary hover:underline">
                          {request.video_room_url}
                        </a>
                        <p className="text-xs text-muted-foreground">This video link expires 2 hours after creation. Expiry: {formatDate(request.video_room_expires_at)}</p>
                        <div className="grid gap-2 sm:grid-cols-2">
                          <button onClick={() => copyLink(request.video_room_url!)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted">
                            <Copy className="h-3.5 w-3.5" /> Copy Link
                          </button>
                          <a href={whatsappLink(request.phone, request.video_room_url)} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl border border-success/20 bg-success/10 px-3 py-2 text-xs font-semibold text-success hover:bg-success/15">
                            <MessageCircle className="h-3.5 w-3.5" /> Send via WhatsApp
                          </a>
                          <button onClick={() => updateStatusMutation.mutate({ requestId: request.id, status: 'started' })} className="inline-flex items-center justify-center gap-2 rounded-xl border border-warning/20 bg-warning/10 px-3 py-2 text-xs font-semibold text-warning hover:bg-warning/15">
                            <PlayCircle className="h-3.5 w-3.5" /> Mark Started
                          </button>
                          <button onClick={() => updateStatusMutation.mutate({ requestId: request.id, status: 'completed' })} className="inline-flex items-center justify-center gap-2 rounded-xl border border-success/20 bg-success/10 px-3 py-2 text-xs font-semibold text-success hover:bg-success/15">
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setSelectedRequest(null)}>
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-border bg-card p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              ...
              <button onClick={() => setSelectedRequest(null)} className="rounded-lg px-3 py-1 text-sm text-muted-foreground hover:bg-muted">Close</button>
            </div>
